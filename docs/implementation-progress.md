...
Reduced the WPF client to the first smallest buildable milestone only.

Implemented for this milestone:

- fullscreen always-on-top check-in screen
- student ID and name inputs
- fake successful login flow with local mock data only
- simple unlocked state
- logout back to the lock/check-in screen

Removed from the active WPF prototype for now:

- Supabase and API dependencies
- local persistence
- startup recovery
- idle tracking
- session bar window
...
Added milestone-specific documentation:

- `docs/milestone-1-prototype.md`
